...
new Vue({
  ...
  methods: {
    async createContent() {
      ...
      item.id = Date.now();
      const newItem = await request("/api/us", "POST", item);

      this.content.push(newItem);

      this.form.date =
        this.form.benefactor =
        this.form.objective =
        this.form.amount =
          "";
      switchPanel();
     ...
     async mark(id) {
      ...
      this.form.benefactor = item.benefactor;
    ...
    async update() {
      ...
      const updated = await request(`/api/us`, "PUT", { ...this.form });

      item.date = updated.date;
      item.benefactor = updated.benefactor;
    ...
    async removeContent(id) {
      await request(`/api/us/${id}`, "DELETE");
  ...
  async mounted() {
    this.content = await request("/api/us");
